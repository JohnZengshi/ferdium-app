...
export default function enforceMacOSAppLocation(): void {
  ...
  const clickedButtonIndex = api.dialog.showMessageBoxSync({
    ...
    detail:
      'Aitalk must live in the Applications folder to be able to run correctly.',
    buttons: ['Move to Applications folder', 'Quit Aitalk'],
    defaultId: 0,
    cancelId: 1,
  });
    ...
    conflictHandler: conflict => {
      if (conflict === 'existsAndRunning') {
        ...
        api.dialog.showMessageBoxSync({
          type: 'error',
          message:
            'Another version of Aitalk is currently running. Quit it, then launch this version of the app again.',
          buttons: ['OK'],
        });
