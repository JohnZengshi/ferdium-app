...
import { TIKTOK_RECIPE_ID } from '../../config';
...
const messages = defineMessages({
  tabAll: {
    id: 'tiktokAccountSlider.tabAll',
    defaultMessage: 'All',
  },
  bindAccount: {
    id: 'tiktokAccountSlider.bindAccount',
  ...
  addSuccess: {
    id: 'tiktokAccountMgmt.addSuccess',
...
interface ITikTokAccountSliderState {
  isBindDrawerVisible: boolean;
  editingService: ServiceDrawerData | null;
  bindDrawerKey: number;
}

interface TikTokSliderItemProps {
  service: Service;
  actions?: Actions;
  onContextMenu: (service: Service) => void;
}

const TikTokSliderItem = SortableElement<TikTokSliderItemProps>(
  observer(({ service, actions, onContextMenu }: TikTokSliderItemProps) => (
...
interface TikTokSliderListProps {
  services: Service[];
  actions?: Actions;
  onContextMenu: (service: Service) => void;
...
const TikTokSliderList = SortableContainer<TikTokSliderListProps>(
  observer(({ services, actions, onContextMenu }: TikTokSliderListProps) => (
...
@observer
class TikTokAccountSlider extends Component<IProps, ITikTokAccountSliderState> {
  ...
  onSortEnd = ({
    ...
    const filtered = stores?.services?.tiktokServices ?? [];
  ...
  render(): ReactElement {
    ...
    const tiktokServices = stores?.services?.tiktokServices ?? [];

    return (
      <ResizableSidebar
        defaultWidth={stores?.settings.all.app.tiktokAccountSliderWidth ?? 300}
        onWidthChange={width =>
          actions?.settings.update({
            type: 'app',
            data: { tiktokAccountSliderWidth: width },
          })
        }
      >
        <div className="flex flex-row items-start gap-[9px] h-fit flex-shrink-0 w-full">
          <Badge
            count={tiktokServices.length > 0 || null}
            size="small"
            offset={[10, 0]}
